...
# def loadDataSet(train_rate):
#     vocabularySet = set()
#
#     # 1.加载文档
#     pos_doc, vocabularySet = loadDataSet2List("result/positive.txt", vocabularySet)
#     print len(pos_doc), len(vocabularySet)
#
#     neg_doc, vocabularySet = loadDataSet2List("result/negative.txt", vocabularySet)
#     print len(neg_doc), len(vocabularySet)
#
#     midum_doc, vocabularySet = loadDataSet2List("result/midum.txt", vocabularySet)
#     print len(midum_doc), len(vocabularySet)
#
#     # 2.拆分训练集和测试集
#     all_postive_doc_len = len(pos_doc)
#     all_negative_doc_len = len(neg_doc)
#     all_midum_doc_len = len(midum_doc)
#     train_pos_len = int(all_postive_doc_len * train_rate)  # 拆分比例
#     train_neg_len = int(all_negative_doc_len * train_rate)
#     train_midum_len = int(all_midum_doc_len * train_rate)
#
#     train_dataset = pos_doc[:train_pos_len] + neg_doc[:train_neg_len] + midum_doc[:train_midum_len]
#     train_label = [1 for i in range(len(pos_doc[:train_pos_len]))] + [0 for i in range(len(neg_doc[:train_neg_len]))] + [2 for i in range(len(midum_doc[:train_midum_len]))]
#
#
#
#     print len(train_dataset), len(train_label), train_pos_len, train_neg_len, train_midum_len
#
#     # test_dataset = pos_doc[train_pos_len:] + neg_doc[train_neg_len:]
#     # test_label = [1 for i in range(len(pos_doc[train_pos_len:] ))] + [0 for i in range(len(neg_doc[train_neg_len:]))]
#     my_len = 50
#     test_dataset = pos_doc[-my_len:] + neg_doc[-my_len:] + midum_doc[-my_len:]
#     test_label = [1 for i in range(len(pos_doc[-my_len:]))] + [0 for i in range(len(neg_doc[-my_len:]))] + [2 for i in range(len(midum_doc[-my_len:]))]
#     print len(test_dataset), len(test_label)
#
#     return train_dataset, train_label, test_dataset, test_label, vocabularySet
