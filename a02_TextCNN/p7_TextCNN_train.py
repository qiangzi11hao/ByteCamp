...
tf.app.flags.DEFINE_string("traning_data_path","../data/data1.txt","path of traning data.") #sample_multiple_label.txt-->train_label_single100_merge
...
tf.app.flags.DEFINE_float("decay_rate",0.99, "Rate of decay for learning rate.") #0.65一次衰减多少
...
tf.app.flags.DEFINE_string("word2vec_model_path","../data/pretrain.vec","word2vec's vocabulary and vectors")
...
def main(_):
    ...
    train, test= load_data(FLAGS.traning_data_path,vocabulary_word2index, vocabulary_label2index,FLAGS.sentence_len)
    trainX, trainY = train
    testX, testY = test
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                if start%(2000*FLAGS.batch_size)==0: # eval every 3000 steps.
                    ...
                    save_path = FLAGS.ckpt_dir + "model_.ckpt"
            ...
            if epoch % FLAGS.validate_every==0:
                ...
                print("Epoch %d Validation Loss:%.3f\tF1 Score:%.3f\tPrecision:%.3f\tRecall:%.3f" % (epoch,eval_loss,f1_score,precision,recall))
                #save model to checkpoint
                save_path=FLAGS.ckpt_dir+"model_embed.ckpt"
                saver.save(sess,save_path,global_step=epoch)

        # 5.最后在测试集上做测试，并报告测试准确率 Test
        test_loss,_,test_precision,_ = do_eval(sess, textCNN, testX, testY,iteration)
...
def do_eval(sess,textCNN,evalX,evalY,iteration):
    ...
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        ...
        f1_score,p,r=compute_f1_score(list(label_list_top5), evalY[start:end])
